...
\section{\texorpdfstring{行列式为 \(0\) 的阵的性质}%
{行列式为 0 的阵的性质}}
...
\begin{proof}
    ...
    故有非零的 \(n \times 1\)~阵 \(X\),
    使 \(A X = 0\)
    (见第~1~章, 节~\malneprasekcio{23}, % tex-fmt: skip
    或第~1~章, 节~\malneprasekcio{25},
    或 ``REF 阵、线性方程组、秩''). % tex-fmt: skip
...
\begin{proof}
    ...
    \(|[X]_{j,1}| \geq |[X]_{k,1}| \geq |[X]_{p,1}|\),
    且 \(|[X]_{j,1}| > 0\).
    ...
    则
    (取 \(u = j\), % tex-fmt: skip
    并注意, 对任何不超过 \(n\), 且不等于 \(j\) 的正整数 \(p\),
    \(|[X]_{p,1}| \leq |[X]_{k,1}|\)) % tex-fmt: skip
    \begin{align*}
        |[A]_{j,j}|\, |[X]_{j,1}|
        \leq {} &
    ...
    (取 \(u = k\), % tex-fmt: skip
    并注意, 对任何不超过 \(n\) 的正整数 \(q\),
    \(|[X]_{q,1}| \leq |[X]_{j,1}|\)), % tex-fmt: skip
    \begin{align*}
        |[A]_{k,k}|\, |[X]_{k,1}|
        \leq {} &
    ...
    \end{align*}
    故 \(|[A]_{j,j}| = 0\).
    则式~\eqref{eq:NonzeroDet2} 的左边是 \(0\).
\end{proof}
...
最后, 注意, 一个方阵与它的转置的行列式相等:
若 \(\det {(A)} = 0\),
则 \(\det {(A^{\mathrm{T}})} = \det {(A)} = 0\).
应用前 \(2\)~个定理于 \(A^{\mathrm{T}}\), 立得
...
\section{\texorpdfstring{判断阵的行列式不是 \(0\) 的方法}%
{判断阵的行列式不是 0 的方法}}

我们知道, 若一个阵的行列式是 \(0\),
则形如式~\eqref{eq:NonzeroDet1}, \eqref{eq:NonzeroDet2} 的不等式成立.
利用反证法, 我们立得判断阵的行列式不是 \(0\) 的方法:
...
\begin{example}\label{emp:NonzeroDet3}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    注意, 在后面的计算中, \(c_i\) 会常被取绝对值.
    既然如此, 为方便, 我们无妨要求 \(c_i > 0\).
...
\section{\texorpdfstring{判断实阵的行列式大于 \(0\) 的方法}%
{判断实阵的行列式大于 0 的方法}}
...
%     故 \(f\) 是连续函数.
%     注意, \(f(0) = [A]_{1,1} [A]_{2,2} \dots [A]_{n,n} > 0\);
%     再注意, \(f(1) = \det {(A)}\).
...
%     加 \(B_1\) 的行~\(1\) 的 \(-[A]_{2,1} / [A]_{1,1}\)~倍%
%     于 \(B_1\) 的行~\(2\), 得 \(n\)~级实阵 \(B_2\).
%     则
%     \begin{align*}
%         [B_2]_{i,j}  & = [B_1]_{i,j},
%                      & \quad i < 2;                        \\
...
%     加 \(B_2\) 的行~\(1\) 的 \(-[A]_{3,1} / [A]_{1,1}\)~倍%
%     于 \(B_2\) 的行~\(3\), 得 \(n\)~级实阵 \(B_3\).
%     则
%     \begin{align*}
%         [B_3]_{i,j}  & = [B_2]_{i,j},
%                      & \quad i < 3;                        \\
...
%     加 \(B_{n-1}\) 的行~\(1\) 的 \(-[A]_{n,1} / [A]_{1,1}\)~倍%
%     于 \(B_{n-1}\) 的行~\(n\), 得 \(n\)~级实阵 \(B_n\).
%     则
%     \begin{align*}
...
% Note that one can also consider the determinant of
%     \begin{align*}
%         [A_t]_{i,j} =
%         \begin{cases}
%             [A]_{i,i},   & 1 \leq i = j \leq n; \\
...
% using some facts about continuous functions.
...
This material presents some sufficient conditions
that det (A) be nonzero (for a complex square matrix A)
or be greater than zero (for a real square matrix A).
The theorem that
A X = 0 has a nonzero solution if det (A) = 0
is used,
but it is proved at least two times
(in sections 23 and 25 of chapter 1).
It is fine that one takes this theorem for granted.

With this theorem, one gets two (or four, if variants are included)
necessary conditions that det (A) be zero
(for a complex square matrix A),
so one immediately gets sufficient conditions
that det (A) be nonzero (for a complex square matrix A).

It is indeed challenging to prove
the two sufficient conditions
that det (A) be greater than zero
(for a real square matrix A).
A fact from calculus is used:
if f: [u, v] \to R is continuous,
and f(t) is nonzero for every t in [u, v],
and f(x) > 0 for some x in [u, v],
then f(t) > 0 for every t in [u, v].
As is "commented",
I know how to prove the two sufficient conditions
in a short and elegant way that uses the above fact.
However, considering that not everyone knows calculus well,
I give (and only use) the following special case:
if f(t) = mt + b for (every) t in [0, 1]
(for some real numbers m and b),
and f(t) is nonzero for every t in [0, 1],
and b = f(0) > 0,
then m + b = f(1) > 0.
Since the case is very weak,
the proofs of the two sufficient conditions
have to be longer.

Note that these theorems are closely related to
the theorem of Gerschgorin (Гершгорин) discs
and the theorem of Brauer's ovals of Cassini,
which can be used to bound the set of eigenvalues
of a (complex) square matrix.
However, I do not even mention in my book
what an eigenvalue is,
so I can only leave my remark here.

...
\begin{example}
    ...
    \begin{align*}
        ...
        \det {\left(B\submat{1,2}{1,2}\right)}
        + \det {\left(A\submat{1,2}{1,3}\right)}
        \det {\left(B\submat{1,3}{1,2}\right)}
        \\
        &
        + \det {\left(A\submat{1,2}{2,3}\right)}
        \det {\left(B\submat{2,3}{1,2}\right)}.
    \end{align*}
    这里, 注意, 适合条件
    ``\(1 \leq j_1 < j_2 \leq 3\)''
    的 \((j_1, j_2)\)
...
\begin{proof}
    ...
    (\(t = 1\), \(2\), \(\dots\), \(n\)).
    注意, \(f(j_t) = t\).
    并且, 若 \(k_t < k_v\), 则 \(f(k_t) < f(k_v)\).
    反过来, 若 \(f(k_t) < f(k_v)\), 则 \(k_t < k_v\).
    (我用一个简单的例助您理解. % tex-fmt: skip
    设 \(m = 9\), \(n = 3\).
    ...
    故 \(f(k_1) = f(j_2) = f(8) = 2\),
    \(f(k_2) = f(j_3) = f(9) = 3\),
    且 \(f(k_3) = f(j_1) = f(6) = 1\).) % tex-fmt: skip
    从而
...
\begin{theorem}
    ...
    \begin{align*}
        &
        \det {\left(
                D\submat{i_1, i_2, \dots, i_n}{1, 2, \dots, n}
        \right)}
        \\
        = {} &
        \sum_{\substack{
...
\begin{proof}
    设
    \begin{align*}
        H = D\submat{i_1, i_2, \dots, i_n}{1, 2, \dots, n}.
    \end{align*}
    不难看出,
    \([H]_{u,v} = [D]_{i_u,v}\).

...
\begin{definition}
    ...
    \end{align*}
    (通俗地, \(AB\) 是 \(m \times n\)~阵, % tex-fmt: skip
    其的 \((i, j)\)-元是
    \(A\)~的行~\(i\) 跟 \(B\)~的列~\(j\)~的%
    相应位置的元的积的和.) % tex-fmt: skip
\end{definition}

可以看到, 阵的乘法跟阵的加法比, 有大的区别.
首先, 加法要求 \(2\)~个阵的尺寸相同, 而乘法要求%
...
\begin{proof}
    ...
    (2)
    您还是要先说明, 等式的 \(2\)~边的阵的尺寸都是 \(m \times n\)
    (我留它为您的习题).
    根据阵的和与积的定义,
    \begin{align*}
        [(A + D)B]_{i,j}
    ...
    (4)
    先证 \(I_m A = A\).
    您还是要先说明, 等式的 \(2\)~边的阵的尺寸都是 \(m \times s\)
    (我留它为您的习题).
    回想, 单位阵 \(I\) 的 \((i, j)\)-元是
    \(\delta(i, j)\);
    具体地, \(i = j\) 时, 它是 \(1\),
...
\begin{proof}
    ...
    (2)
    \(f\) 是交错性的.
    因为若 \(c_1\), \(c_2\), \(\dots\), \(c_n\) 中有 \(2\)~个相等,
    则 \(Bc_1\), \(Bc_2\), \(\dots\), \(Bc_n\) 中也有 \(2\)~个相等.
    再利用行列式的交错性,
    \(f(C) = \det {(BC)} = 0\).
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    (通俗地, \(B\)~的列~\(v\) 等于 \(A\)~的列~\(v\) % tex-fmt: skip
    (\(v \neq k\)),
    而 \(B\)~的列~\(k\) 的元分别是
    \(z_1\), \(z_2\), \(\dots\), \(z_n\).) % tex-fmt: skip
...
一个 \(1\)~级阵 \(A = [a]\) 的古伴
\(\operatorname{adj} {(A)} =
[(-1)^{1+1} \allowbreak \cdot \det {(A(1|1))}] = [1]\),
即 \(1\)~级单位阵.

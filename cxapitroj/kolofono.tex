...
the OpenType format version of
Source Han Serif (designed by Adobe and Google),
and the TrueType format version of
LXGW WenKai Mono (designed by
\href{https://github.com/lxgw/LxgwWenKai}{LXGW}).
...
(or \url{https://gitee.com/septsea/det}).
...
\begin{longtable}[c]{llllll}
    \multicolumn{6}{c}{\textbf{The Greek alphabet}}
    \\[2ex]
    \multicolumn{2}{c}{letter}
    &
    \multicolumn{2}{c}{name}
    &
    \multicolumn{2}{c}{pronunciation}
    \\ \hline
    \endfirsthead

    \multicolumn{6}{r}{(continued from the previous page)}
    \\[2ex]
    \multicolumn{2}{c}{letter}
    &
    \multicolumn{2}{c}{name}
    ...
    Α     & α     & άλφα    & alpha   & \textit{AL-fuh}                   & \textsf{a'erfa}     \\
    Β     & β     & βήτα    & beta    & \textit{BAY-tuh}                  & \textsf{beita}      \\
    Γ     & γ     & γάμμα   & gamma   & \textit{GAM-muh}                  & \textsf{gama}       \\
    Δ     & δ     & δέλτα   & delta   & \textit{DEL-tuh}                  & \textsf{de'erta}    \\
    Ε     & ε (ϵ) & έψιλον  & epsilon & \textit{EP-suh-lon}               & \textsf{aipuxilong} \\
    Ζ     & ζ     & ζήτα    & zeta    & \textit{ZAY-tuh}                  & \textsf{zeta}       \\
    Η     & η     & ήτα     & eta     & \textit{AY-tuh}                   & \textsf{yita}       \\
    Θ (ϴ) & θ (ϑ) & θήτα    & theta   & \textit{THAY-tuh}                 & \textsf{xita}       \\
    Ι     & ι     & ιώτα    & iota    & \textit{eye-OH-tuh}               & \textsf{yueta}      \\
    Κ     & κ (ϰ) & κάππα   & kappa   & \textit{KAP-uh}                   & \textsf{kapa}       \\
    Λ     & λ     & λάμδα   & lambda  & \textit{LAM-duh}                  & \textsf{lamuda}     \\
    Μ     & μ     & μι      & mu      & \textit{mew}                      & \textsf{miu}        \\
    Ν     & ν     & νι      & nu      & \textit{new}                      & \textsf{niu}        \\
    Ξ     & ξ     & ξι      & xi      & \textit{ksigh}                    & \textsf{kexi}       \\
    Ο     & ο     & όμικρον & omicron & \textit{OM-uh-CRON}               & \textsf{aomikerong} \\
    Π     & π (ϖ) & πι      & pi      & \textit{pie}                      & \textsf{pai}        \\
    Ρ     & ρ (ϱ) & ρο      & rho     & \textit{row}                      & \textsf{rou}        \\
    Σ     & σ/ς   & σίγμα   & sigma   & \textit{SIG-muh}                  & \textsf{xigema}     \\
    Τ     & τ     & ταυ     & tau     & \textit{tow} (as in \textit{cow}) & \textsf{tao}        \\
    Υ     & υ     & ύψιλον  & upsilon & \textit{OOP-suh-LON}              & \textsf{yupuxilong} \\
    Φ     & φ (ϕ) & φι      & phi     & \textit{fi} (as in \textit{hi})   & \textsf{fei}        \\
    Χ     & χ     & χι      & chi     & \textit{ki} (as in \textit{hi})   & \textsf{xi}         \\
    Ψ     & ψ     & ψι      & psi     & \textit{psigh}                    & \textsf{pusai}      \\
    Ω     & ω     & ωμέγα   & omega   & \textit{oh-MAY-guh}               & \textsf{aomiga}     \\
\end{longtable}

...
(1)
存在数 \(0\), 使对任何数 \(a\),  \(0 + a = a = a + 0\);

(2)
对任何数 \(a\), 存在数 \(-a\), 使 \((-a) + a = 0 = a + (-a)\).
...
(5)
存在数 \(1\), 使对任何数 \(a\), \(1a = a = a1\);

(6)
对任何非零的数 \(a\), 存在数 \(a^{-1}\), 使 \(a^{-1} a = 1 = a a^{-1}\).

设 \(x \neq 0\), 且 \(xy = xz\).
则 \(x^{-1} (xy) = x^{-1} (xz)\).
由结合律, \((x^{-1} x) y = (x^{-1} x) z\).
则 \(1y = 1z\).
则 \(y = z\).
类似地, 可证,
若 \(x \neq 0\), 且 \(yx = zx\), 则 \(y = z\).
...
\begin{theorem}
    ...
    设 \(A\) 有一个行列式非零的 \(n\)~级子阵
    \begin{align*}
        L = A\submat{i_1,i_2,\dots,i_n}{1,2,\dots,n},
    \end{align*}
    ...
    设 \(H\) 有一个行列式非零的 \(n\)~级子阵
    \begin{align*}
        K = H\submat{1,2,\dots,n}{j_1,j_2,\dots,j_n},
    \end{align*}
...
\begin{proof}[证~1]
    ...
    \begin{align*}
        [L B]_{u,j}
        =
        \sum_{p = 1}^{n} {[L]_{u,p} [B]_{p,j}}
        =
        \sum_{p = 1}^{n} {[A]_{i_u,p} [B]_{p,j}}
        =
        [A B]_{i_u,j}.
    \end{align*}
    类似地, \([L C]_{u,j} = [A C]_{i_u,j}\).
    因为 \(A B = A C\),
    我们得 \([L B]_{u,j} = [L C]_{u,j}\).
    注意, \(L B\) 与 \(L C\) 都是 \(n \times m\)~阵.
    则 \(L B = L C\).
    因为 \(\det {(L)} \neq 0\),
    由上个定理, \(B = C\).
\end{proof}
...
不难看出, 若 \(B\) 是 \(n\)~级阵, 且 \(c\) 是数, 则
\(\det {(cB)} = c^n \det {(B)}\).

\begin{theorem}
    设 \(A\) 是 \(n\)~级阵.
    设 \(c\) 是数.
    则 \(\operatorname{adj} {(cA)}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \\
        = {} &
        (-1)^{j+i} c^{n-1} \det {(A (j|i))}
        \\
        = {} &
        c^{n-1} (-1)^{j+i} \det {(A (j|i))}
        \\
        = {} &
        c^{n-1}\, [\operatorname{adj} {(A)}]_{i,j}
        \\
        = {} &
        [c^{n-1} \operatorname{adj} {(A)}]_{i,j}.
        \qedhere
...
\begin{theorem}
    ...
    \begin{equation}
        \begin{aligned}
            & \det {\left(
                    (\operatorname{adj} {(A)})
                    \submat{i_1,\dots,i_k}{j_1,\dots,j_k}
            \right)}
            \\
            = {} &
            (\det {(A)})^{k-1}\,
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
通俗地, \(B\) 的列~\(j_1\), \(\dots\), \(j_k\)
分别与\(\operatorname{adj} {(A)}\)
的列~\(j_1\), \(\dots\), \(j_k\)
相等,
...
\begin{align*}
    ...
    = B\submat{i_{k+1},\dots,i_n}{j_{k+1},\dots,j_n}
    = I_{n-k}.
\end{align*}
% 其中, \(I_{n-k}\) 指 \(n-k\)~级单位阵.

按列~\(j_{k+1}\) 展开 \(\det {(B)}\), 得
\begin{align*}
...
\begin{align*}
    ...
    = (AB)\submat{j_{k+1},\dots,j_n}{j_{k+1},\dots,j_n}
...
若 \(\det {(A)} \neq 0\),
我们可在等式的 \(2\)~边消去它, 得式~\eqref{eq:C2401}.
不过, 若 \(\det {(A)} = 0\),
我们应如何作?
...
\begin{proof}
    ...
    \begin{align*}
        m_j =
        (-[\operatorname{adj} {(A)}]_{t,j})\,
        (-[\operatorname{adj} {(A)}]_{t,s})^{-1}.
    \end{align*}
    则
    \begin{align*}
        C (m_j y_s)
...
\begin{proof}
    ...
    = (\det {(A)})^{n-1} \neq 0\).
    由消去律,
    我们可在等式的 \(2\)~边消去 \(\operatorname{adj} {(A)}\),
    得到结论.
...
\begin{theorem}
    ...
    \begin{align*}
        ...
        \begin{cases}
            [A_{1,1}]_{i,j},
            & \text{\(i \leq r_1\), 且 \(j \leq u_1\)}; \\
            [A_{1,2}]_{i,j-u_1},
            & \text{\(i \leq r_1\), 且 \(j > u_1\)};    \\
            [A_{2,1}]_{i-r_1,j},
            & \text{\(i > r_1\), 且 \(j \leq u_1\)};    \\
    ...
    \begin{align*}
        ...
        \begin{cases}
            [B_{1,1}]_{i,j},
            & \text{\(i \leq u_1\), 且 \(j \leq c_1\)}; \\
            [B_{1,2}]_{i,j-c_1},
            & \text{\(i \leq u_1\), 且 \(j > c_1\)};    \\
            [B_{2,1}]_{i-u_1,j},
            & \text{\(i > u_1\), 且 \(j \leq c_1\)};    \\
    ...
    \begin{align*}
        ...
        \begin{cases}
            [C_{1,1}]_{i,j},
            & \text{\(i \leq r_1\), 且 \(j \leq c_1\)}; \\
            [C_{1,2}]_{i,j-c_1},
            & \text{\(i \leq r_1\), 且 \(j > c_1\)};    \\
            [C_{2,1}]_{i-r_1,j},
            & \text{\(i > r_1\), 且 \(j \leq c_1\)};    \\
...
This material discusses classical adjoints (adjugates)
of square matrices.
It is not very interesting.
I even forgot the reason for which I wrote it.
The law of cancellation is useful, though.

Postscript on 2024-10-03:
Well, I forgot that I discussed
the multiplication of two block matrices,
each of which is partitioned into four blocks.
This section does not seem to be really used elsewhere anyway,
but I decide still to keep it.

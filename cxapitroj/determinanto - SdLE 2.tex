...
\begin{example}
    ...
    则 \(A\) 的每个 \(4\)~级子阵的行列式都是 \(0\).
    从而, 存在一个%
    低于 \(4\) 的正整数 \(r\)
    适合性质:
    \(A\) 有一个 \(r\)~级子阵, 其的行列式非零,
    且 \(A\) 的每个 \(r+1\)~级子阵的行列式都是 \(0\).
    (我们已知 \(A\) 的每个 \(4\)~级子阵的行列式都是 \(0\). % tex-fmt: skip
    我们考虑 \(A\) 的 \(3\)~级子阵.
    若有一个 \(3\)~级子阵的行列式不是 \(0\),
    我们就取 \(r = 3\).
    若不然, \(A\) 的每个 \(3\)~级子阵的行列式都是 \(0\).
    我们考虑 \(A\) 的 \(2\)~级子阵.
    若有一个 \(2\)~级子阵的行列式不是 \(0\),
    我们就取 \(r = 2\).
    若不然, \(A\) 的每个 \(2\)~级子阵的行列式都是 \(0\).
    我们考虑 \(A\) 的 \(1\)~级子阵.
    因为 \(A \neq 0\),
    故 \(A\) 有 \(1\)~级子阵的行列式不是 \(0\).
    此时, 取 \(r = 1\) 即可.) % tex-fmt: skip
    ...
    与列~\(1\), \(2\) 作成的
    \(2\)~级子阵的行列式不是 \(0\),
    但 \(M\) 的每个 \(3\)~级子阵的行列式都是 \(0\)); % tex-fmt: skip
    \(r = 3\) 或 \(r = 1\) 时的情形是类似的.

    我们设
    \(A_2 = A\submat{1,2}{1,2}\)
    的行列式不是 \(0\),
    且 \(A\) 的每个 \(3\)~级子阵的行列式都是 \(0\).
    再设
    \(E = A\submat{1,2,3}{1,2,3}\).
    因为 \(E\) 是 \(A\) 的一个 \(3\)~级子阵,
    故 \(\det {(E)} = 0\).
    从而
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    (通俗地, 我们在 \(T\) 的最后一个元后加 \(2\)~个 \(0\), % tex-fmt: skip
    变 \(3 \times 1\)~阵 \(T\)
    为一个 \(5 \times 1\)~阵 \(W\).) % tex-fmt: skip
    因为 \([W]_{3,1} \neq 0\),
    故 \(W \neq 0\).
    我们证明 \(AW = 0\).
...
\begin{proof}
    ...
    若 \(\operatorname{adj} {(A)} = 0\),
    则 \(A\) 的每个 \(n-1\)~级子阵的行列式都是 \(0\)
    (同时, 这也要求 \(n > 2\): % tex-fmt: skip
    回想 \(2\)~级阵的古伴是何;
    再回想前面作过的假定 \(A \neq 0\)). % tex-fmt: skip
    从而, 存在一个%
    低于 \(n-1\) 的正整数 \(r\)
    ...
    \(A\) 有一个 \(r\)~级子阵, 其的行列式非零,
    且 \(A\) 的每个 \(r+1\)~级子阵的行列式都是 \(0\).
    (我们已知 \(A\) 的每个 \(n-1\)~级子阵的行列式都是 \(0\). % tex-fmt: skip
    我们考虑 \(A\) 的 \(n-2\)~级子阵.
    若有一个 \(n-2\)~级子阵的行列式不是 \(0\),
    我们就取 \(r = n-2\).
    若不然, \(A\) 的每个 \(n-2\)~级子阵的行列式都是 \(0\).
    我们考虑 \(A\) 的 \(n-3\)~级子阵.

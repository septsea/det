...
\begin{example}\label{emp:resultant}
    ...
    \begin{align*}
        ...
        = {} &
        \sum_{\ell = 1}^{m + n}
        {[S]_{i,\ell} [\operatorname{adj} {(S)}]_{\ell,m+n}}
        \\
        = {} &
        [S \operatorname{adj} {(S)}]_{i,m+n}
        \\
        = {} &
        [\det {(S)}\, I_{m+n}]_{i,m+n}
        \\
        = {} &
        \det {(S)}\, [I_{m+n}]_{i,m+n}
        \\
    ...
    \begin{align*}
        ...
        {[S]_{\ell,j} x^{m+n-\ell}}.
    \end{align*}
    若 \(j \leq m\), 则 \([S]_{\ell,j} = a_{\ell-j}\).
    则
    \begin{align*}
        [X S]_{1,j}
        = {} &
    ...
    \begin{align*}
        ...
        = {} &
        \sum_{j = 1}^{m + n}
        {[X]_{1,j} [\det {(S)}\, e]_{j,1}}
        \\
        = {} &
        \sum_{j = 1}^{m + n - 1}
        {[X]_{1,j} [\det {(S)}\, e]_{j,1}}
        +
        [X]_{1,m+n} [\det {(S)}\, e]_{m+n,1}
        \\
        = {} &
        0 + 1 \det {(S)}
        \\
    ...
    \end{align*}
    故
    \(f(x)\, u(x) + g(x)\, v(x) = \det {(S)}\).

    (4)
    设数 \(c\) 适合 \(f(c) = 0 = g(c)\).
    由 (3) 知, 必
    \(0 = f(c)\, u(c) + g(c)\, v(c) = \det {(S)}\)
    (我们代 \(x\) 以数 \(c\)).
    这有时是有用的.
\end{example}
...
\begin{example}
    ...
    设 \(n = 0\).
    则 \([S(f, g; 0, m)]_{i,j} = a_{i-j}\).
    则 \(S(f, g; 0, m) = a_0 I_m\).
    则 \(\det {(S(f, g; 0, m))} = a_0^m\).

    直接地算 \(\det {(S(f, g; n, m))}\) 或许是难的.
    为此, 设 \(t\) 是数, 且
    \begin{align*}
        h(x)
        = f(x)\, (x - t)
    ...
    \end{align*}
    于是, 当 \(i = m + n + 1\) 时,
    \([S_2]_{i,j} = a_{n+1+(m-j)} = 0\)
    (若 \(j \leq m\); 注意, \(n+1+(m-j) > n\)),
    且
    \([S_2]_{i,j} = b_{m+1+(m+n-j)} = 0\)
...
\begin{theorem}
    ...
    \end{align*}
    我们说,
    \(S(f, g; n, m)\) 是 \(f\), \(g\) 的 Sylvester 阵,
    且 \(\det {(S(f, g; n, m))}\)
    是 \(f\), \(g\) 的 \angla{resultant},
    ``结式''.
    ...
    (3)
    若 \(n = 0\), 则 \(S(f, g; 0, m) = a_0 I_m\).
    故 \(\det {(S(f, g; 0, m))} = a_0^m\).
    ...
    (6)
    若 \(g(x) = b_0 (x - y_1) (x - y_2) \dots (x - y_m)\),
    则由 (2) (5),
    \begin{align*}
    ...
    (7)
    设 \(e\) 是 \(m + n\)~级单位阵的列~\(m + n\).
    则有 (非零的) \((m + n) \times 1\)~阵 \(p\),
    使 \(S(f, g; n, m)\, p = \det {(S(f, g; n, m))}\, e\).
    (关于 (7) (8) (9) (10), 见例~\ref{emp:resultant}.)

    (8)
    作 \(1 \times (m + n)\)~阵 \(X_{m+n}\),
    使 \([X_{m+n}]_{1,j} = x^{m+n-j}\).
    则 \(X_{m+n}\, S(f, g; n, m)\) 是 \(1 \times (m + n)\)~阵,
    且
    \begin{align*}
        [X_{m+n}\, S(f, g; n, m)]_{1,j}
        = \begin{cases}
              f(x)\, x^{m-j},     & j \leq m; \\
              g(x)\, x^{n-(j-m)}, & j > m.
          \end{cases}
    ...
    \begin{align*}
        u(x)
        = \sum_{k = 0}^{m-1} {[Y]_{k+1,1} x^{m-1-k}}
        = {} &
        [Y]_{1,1} x^{m-1} + [Y]_{2,1} x^{m-2}
        + \dots + [Y]_{m,1},
        \\
        v(x)
        = \sum_{k = 0}^{n-1} {[Y]_{m+1+k,1} x^{n-1-k}}
        = {} &
        [Y]_{m+1,1} x^{n-1} + [Y]_{m+2,1} x^{n-2}
        + \dots + [Y]_{m+n,1}.
    \end{align*}
    则 \(X_{m+n}\, S(f, g; n, m)\, Y\) 是 \(1\)~级阵, 且
    \begin{align*}
        [X_{m+n}\, S(f, g; n, m)\, Y]_{1,1} = f(x)\, u(x) + g(x)\, v(x).
    \end{align*}
    特别地, 若取 \(Y\) 为 (7) 中的 \(p\),
    则对应的 \(u\), \(v\) 适合,
    \begin{align*}
    ...
    则由 (9),
    \begin{align*}
        0 = f(c)\, u(c) + g(c)\, v(c) = \det {(S(f, g; n, m))}.
    \end{align*}

    (11)
    反过来, 若 \(\det {(S(f, g; n, m))} = 0\),
    则由 (1) (5) (6),
    \(a_0 = 0 = b_0\),
    或当 \(a_0 \neq 0\) 或 \(b_0 \neq 0\) 时,
    存在数 \(c\), 使 \(f(c) = 0 = g(c)\).

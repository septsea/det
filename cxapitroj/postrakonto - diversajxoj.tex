...
\begin{example}
    ...
    \end{align*}
    (\(\operatorname{tr}\) 来自英语 \angla{trace}.)
    则
    \begin{align*}
        \mathrm{D} (\det {(A)})
...
\begin{example}
    ...
    \(B A = 0\) 相当于 \(A^{\mathrm{T}} B^{\mathrm{T}} = 0\).
    ...
    并且, 当然, \(G \neq 0\).
    那么, 由上面的讨论, \(A^{\mathrm{T}} B^{\mathrm{T}} = 0\).
\end{example}

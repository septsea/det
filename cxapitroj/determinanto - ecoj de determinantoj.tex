...
显然, \(\delta\)-记号是表示 \(2\)~个文字是否相等的一个量.
\(\delta(i, j) = 1\), 说明 \(i\), \(j\) 是同一个文字;
\(\delta(i, j) = 0\), 说明 \(i\), \(j\) 是不同的 \(2\)~个文字.
...
\(I_n\) 的列 \(1\), \(2\), \(\dots\), \(n\)). % tex-fmt: skip
记 \(k_i = [x]_{i,1}\).
则
\begin{align*}
    x
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \\
        = {} & \det {(I_{m-1})}
        + \sum_{i = 2}^{m} {(-1)^{i+1} 0 \det {(I_m (i|1))}}
        \\
        = {} & \det {(I_{m-1})}
        \\
        = {} & 1.
...
\begin{proof}
    ...
    (具体地, 设交换 \(n\)~级阵 \(A\)~的% % tex-fmt: skip
    列~\(p\) 与列~\(q\) 后得到的阵为 \(B\),
    且 \(p < q\),
    则 \(g(B) = -g(A)\);
    这是前 \(2\)~个性质的推论.) % tex-fmt: skip
    不难看出, \(g(I) = 0\).
    ...
    \begin{align*}
        a_{k}
        = [A]_{1,k} e_{1} + [A]_{2,k} e_{2} + \dots + [A]_{n,k} e_{n}
    ...
    所以 \(g(A) = 0\).
    故 \(f(A) = f(I) \det {(A)}\).
    (反过来, 不难验证, 若我们定义 % tex-fmt: skip
    \(f(A) = f(I) \det {(A)}\),
    则 \(f\) 适合多线性与交错性.) % tex-fmt: skip
\end{proof}
...
\begin{restatable}{theorem}{TheoremExpansionAboutRowOne}
    ...
    \begin{align*}
        \det {(A)}
        = \sum_{j = 1}^{n} {(-1)^{1+j} [A]_{1,j} \det {(A(1|j))}}.
    \end{align*}
\end{restatable}

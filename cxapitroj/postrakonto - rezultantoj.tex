...
\begin{theorem}
    ...
    \begin{align*}
        \mathrm{D} f(x) = a_0
        \sum_{1 \leq v \leq n} {L_v (x)},
    \end{align*}
    且对不超过 \(n\) 的正整数 \(i\),
    \(\mathrm{D} f(x_i) = a_0 L_i (x_i)\),
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        \mathrm{D} f(x)
        = {} &
        (\mathrm{D} p_0 (x)) p_1 (x) \dots p_n (x)
        + p_0 (x) (\mathrm{D} p_1 (x)) \dots p_n (x)
        \\
        &
        \quad
        + \dots
        + p_0 (x) p_1 (x) \dots (\mathrm{D} p_n (x)).
        \\
        = {} &
        0
        + a_0 \prod_{\substack{1 \leq j \leq n \\ j \neq 1}}
        {(x - x_j)}
        + \dots
        + a_0 \prod_{\substack{1 \leq j \leq n \\ j \neq n}}
        {(x - x_j)}
        \\
        = {} &
        a_0 \sum_{1 \leq v \leq n} {L_v (x)}.
    \end{align*}
    最后, 注意, 若 \(i \neq v\), 则 \(L_v (x_i) = 0\).
    则由上式, \(\mathrm{D} f(x_i) = a_0 L_i (x_i)\).
\end{proof}
...
\begin{example}
    ...
    \begin{align*}
        ...
        (x_1 x_2 \dots x_n)
        \prod_{i = 1}^{n}
        {\left(z - \frac{1}{x_i}\right)}
        \\
        = {} &
        a_0\, (x_1 x_2 \dots x_n)
        \prod_{i = 1}^{n}
        {\left(
                \frac{-z}{x_i} \left(\frac{1}{z} - x_i\right)
        \right)}
        \\
        = {} &
        (x_1 x_2 \dots x_n)
        \frac{(-z)^n}{x_1 x_2 \dots x_n}
        a_0 \prod_{i = 1}^{n}
        {\left(\frac{1}{z} - x_i\right)}
        \\
        = {} &
        (-1)^n z^n f\left(\frac{1}{z}\right).
        \qefhere
    \end{align*}
\end{example}
...
This material also discusses the derivative of a polynomial.
